...
## 開発・運用メモ
### テスト
判定ロジックを触ったら必ず実行してください（ネットワーク不要・約1秒）。
...
判定は **テクニカル + ファンダ** の合成スコア（-1〜+1）で行います。
配分はモード依存で、短期（scalp/day）は **テク85% : ファンダ15%**、swing は **45% : 55%**（ファンダ重視）。
...
### モード切替（scalp / day）
| モード | 足 | EMA | RSI | SL | TP | しきい値 | テク:ファンダ | 有効期限 |
|---|---|---|---|---|---|---|---|---|
| scalp | 1分 | 5/13 | 7 | ATR×1.0 | SL×1.5 | 0.35 | 85:15 | 3分 |
...
- **スコア・シグナル・推奨TP/SL・エントリー有効帯**：約45秒ごとに再計算（scalp=1分足 / day=15分足 / swing=1時間足）
- ヘッダに「● LIVE計算 時刻」を表示
